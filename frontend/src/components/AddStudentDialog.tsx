...
import { departments, years, classificationLabels } from "@/data/constants";
...
const AddStudentDialog = () => {
  ...
  const [form, setForm] = useState({
    ...
    email: "", phone: "", rollNo: "", attendancePercentage: "75",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const attendancePercentage = Math.max(0, Math.min(100, Number(form.attendancePercentage || 0)));
    ...
    try {
      ...
      await addStudent({
        ...form,
        engagementLevel,
        attendance: { percentage: attendancePercentage, presentDays: 120, totalDays: 160, leaveCount: 5 },
        academics: {
      ...
      setForm({ name: "", department: departments[0], year: years[0], classification: "intermediate", mentor: mentors[0], warden: wardens[0], residenceType: "Hosteller", email: "", phone: "", rollNo: "", attendancePercentage: "75" });
      setOpen(false);
    } catch (err) {
      toast({
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={e => updateField("attendancePercentage", e.target.value)}
                placeholder="75"
              />
            </div>
          </div>
